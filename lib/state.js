import { LineBreak, classes, values } from './LineBreak.js';
...
export class BreakerState {
  ...
  prev = new BreakerStep(sot, -Infinity, '', 0);
  cur = new BreakerStep(sot, -Infinity, '', 0);
  next = new BreakerStep(sot, -Infinity, '', 0);
  ...
  pushEnd() {
    this.push(new BreakerStep(eot, Infinity, '', this.next.len));
  }
  ...
  * codePoints(pos) {
    while (pos < this.len) {
      if ((pos === this.cur.len) && (this.next.cls >= 0)) { // Not sot or eot
        ...
        yield new BreakerStep(resolve(cls, char), cp, char, pos);
      }
    }
  }
