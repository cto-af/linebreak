...
export class LineBreaker {
  ...
  static* #charClasses(str) {
    ...
    for (const [cp, len] of LineBreaker.#codePoints(str)) {
      yield [mapClass(/** @type {number} */(LineBreak.get(cp))), pos];
      pos += len;
    }
  }
   ...
   * @returns {boolean?}
   */
  static #getSimpleBreak(curClass, nextClass) {
  ...
  #getPairTableBreak(lastClass) {
    // Ff not handled already, use the pair table
  ...
  * breaks(str) {
    ...
    for (const [cls, pos] of LineBreaker.#charClasses(str)) {
      ...
      let shouldBreak
        = LineBreaker.#getSimpleBreak(this.curClass, this.nextClass);
